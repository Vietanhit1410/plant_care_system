...
export function renderPlantHistory({ root, items = [], onClose }) {
  ...
  root.innerHTML = `
    <section class="modal__panel plant-history">
      <div class="modal__header plant-history__header">
        ...
        <button class="modal__close" type="button" data-close-history aria-label="Đóng lịch sử">&times;</button>
      </div>
      <div class="timeline plant-history__timeline">
        ${historyItems.map((item, index) => `
          <div class="timeline__item plant-history__item" data-history-idx="${index}" style="cursor: pointer; transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
  ...
  const closeButton = root.querySelector('[data-close-history]');
  if (closeButton && typeof onClose === 'function') {
    closeButton.addEventListener('click', onClose);
  }

  const timelineEls = root.querySelectorAll('[data-history-idx]');
